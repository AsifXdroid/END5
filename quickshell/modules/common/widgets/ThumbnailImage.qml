...
StyledImage {
    ...
    Process {
        ...
        command: {
            ...
            const targetDir = FileUtils.trimFileProtocol(root.thumbnailPath).replace(/\/[^\/]+$/, "");
            return ["bash", "-c",
                `[ -f '${FileUtils.trimFileProtocol(root.thumbnailPath)}' ] && exit 0 || { mkdir -p '${targetDir}' && magick '${root.sourcePath}' -thumbnail ${maxSize}x${maxSize} '${FileUtils.trimFileProtocol(root.thumbnailPath)}' && exit 1; }`
            ]
        }
