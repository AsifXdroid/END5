...
MouseArea {
    ...
    property var quickDirs: [
        { icon: "favorite",   name: "Favorites   ",  path: "__favorites__",   alwaysVisible: Config.options.wallpaperSelector.favorites.length > 0 },
                            ...
                            sourceComponent: RowLayout {
                                ...
                                Repeater {
                                    ...
                                    delegate: RippleButton {
                                        ...
                                        toggled: modelData.path === "__favorites__"
                                            ? gridLoader.item?.showFavorites ?? false
                                            : Wallpapers.directory === Qt.resolvedUrl(modelData.path)
                                        colBackgroundToggled: Appearance.colors.colSecondaryContainer
                                        colBackgroundToggledHover: Appearance.colors.colSecondaryContainerHover
                                        colRippleToggled: Appearance.colors.colSecondaryContainerActive
                                        onClicked: {
                                            if (modelData.path === "__favorites__") {
                                                Wallpapers.setDirectory(`${Directories.pictures}/Wallpapers`);
                                                gridLoader.item.showFavorites = true;
                                            } else {
                                                if (gridLoader.item) gridLoader.item.showFavorites = false;
