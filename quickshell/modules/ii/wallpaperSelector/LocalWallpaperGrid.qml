...
Item {
    ...
    function refreshFavorites() {
        ...
        const favs = Config.options.wallpaperSelector.favorites
        for (let i = 0; i < favs.length; i++) {
            favoritesModel.append({ filePath: favs[i], fileName: favs[i].split("/").pop(), fileIsDir: false })
        }
    }

    onShowFavoritesChanged: {
        if (showFavorites) refreshFavorites()
    }

    Connections {
        target: Config.options.wallpaperSelector
        function onFavoritesChanged() {
            if (root.showFavorites) refreshFavorites()
        }
    }
    ...
    GridView {
        ...
        function moveSelection(delta) {
            ...
            const item = root.showFavorites ? favoritesModel.get(currentIndex) : grid.model;
            const filePath = root.showFavorites ? item.filePath : grid.model.get(currentIndex, "filePath");
            const isDir = root.showFavorites ? false : grid.model.get(currentIndex, "fileIsDir");
            if (!isDir && filePath && Config.options.background.enableWallpaperPreview) Wallpapers.startPreview(filePath);
        }

        function activateCurrent() {
            const filePath = root.showFavorites
                ? favoritesModel.get(currentIndex).filePath
                : grid.model.get(currentIndex, "filePath");
            root.wallpaperSelected(filePath);
        }

        model: root.showFavorites ? favoritesModel : Wallpapers.folderModel
        onModelChanged: { currentIndex = 0; Wallpapers.stopPreview(); }

        delegate: WallpaperDirectoryItem {
            required property var modelData
            required property int index
            fileModelData: root.showFavorites
                ? ({ filePath: modelData.filePath, fileName: modelData.fileName, fileIsDir: false })
                : modelData
            ...
            MouseArea {
                ...
                onClicked: event => {
                    if (event.button === Qt.RightButton) {
                        ...
                        contextMenu.targetPath = root.showFavorites ? modelData.filePath : modelData.filePath
