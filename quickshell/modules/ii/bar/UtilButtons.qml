...
Item {
    ...
    Flow {
        ...
        Loader {
            active: Config.options.bar.utilButtons.showOcrButton
            visible: active
            sourceComponent: isMaterial ? ocrM3 : legacyOcr
        }
        Component {
            id: ocrM3
            UtilButton {
                iconText: "document_scanner"
                onClicked: Quickshell.execDetached(["qs", "-p", Quickshell.shellPath(""), "ipc", "call", "region", "ocr"])
            }
        }
        Component {
            id: legacyOcr
